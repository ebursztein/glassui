<script lang="ts">
  ...
  const handleKeydown = (e: KeyboardEvent) => {
    ...
    if (orientation === 'horizontal') {
      ...
      else if (e.key === 'ArrowLeft') size = Math.max(minSize, size - step);
      else if (e.key === 'Home') size = minSize;
      else if (e.key === 'End') size = maxSize;
      else handled = false;
    } else {
      if (e.key === 'ArrowDown') size = Math.min(maxSize, size + step);
      else if (e.key === 'ArrowUp') size = Math.max(minSize, size - step);
      else if (e.key === 'Home') size = minSize;
      else if (e.key === 'End') size = maxSize;
      else handled = false;
    }
...
>
  ...
  <div
    class={cn(
      'relative z-10 shrink-0 flex items-center justify-center transition-colors group focus:outline-none focus:ring-2 focus:ring-[var(--comp-bg)]',
      orientation === 'horizontal'
