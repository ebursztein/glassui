...
| columns | `{ key: string, label: string, sortable?: boolean, align?: "left" | "center" | "right" }[]` | — | Column definitions. |
| sortable | `boolean` | `false` | Enable clicking column headers to sort data. |
| searchable | `boolean` | `false` | Enable a global search input to filter rows. |
| pagination | `boolean` | `false` | Enable paginating data into multiple pages. |
| pageSize | `number` | `10` | Number of rows per page (if pagination is enabled). |
| color | `primary | secondary | accent | destructive | neutral | gradient | info | success | warning | error` | — | Theme color for active states (pagination, sorting icons) |
